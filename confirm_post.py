...
def spider_result(cookie_file,spider_url,referer_url,proxy):
	ch = pycurl.Curl()
	buffer_con = StringIO.StringIO()
	header = [
		"Host: www.bjguahao.gov.cn",
...
def confirm_success(cookie_file,referer_url,proxy):
	spider_url = 'http://www.bjguahao.gov.cn/comm/show_cont.php'	
	result_html = spider_result(cookie_file,spider_url,referer_url,proxy)
...
def this_main(cookie_file,referer_url,data,hidden_data,proxy):
	...
	if soup.find('script'):
		...
		guahao_info_res = confirm_success(cookie_file,confirm_post_res['spider_url'],proxy)
